...
lab03-mcp-server is a Model Context Protocol (MCP) server built with FastMCP featuring dynamic tool loading.
...
2. **Run in Container**:
   ```bash
   docker run -i lab03-mcp-server:latest
   ```
...
kubectl get mcpserver lab03-mcp-server
```